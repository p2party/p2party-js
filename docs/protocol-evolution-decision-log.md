...
_Blog:_ This is still the headline promise, but the honest antagonist changed. Signal amortizes the KEM, Apple throttles it, and Zerion pays for one in every fast cover frame. We are testing the sparse point: do the full standard ratchet advance only when healing needs it, and let an already-running room cell carry it.

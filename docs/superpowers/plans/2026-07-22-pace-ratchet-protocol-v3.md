...
**Architecture:** A handshake at the persistent `main` channel's `onopen` produces a 32-byte secret that seeds a per-`(roomId, peerPublicKey)` Double Ratchet; all messages then encrypt under per-message ratchet keys (protocol-v3). No-PIN rooms seed from identity-mixed ephemeral X25519 (FS, not MITM-resistant); PIN rooms seed from CPace over Ristretto255 with identity + DTLS-fingerprint binding (FS + MITM-resistant). The ratchet advances per logical message; all chunks of a message share one message key (nonce = chunkIndex). Ships **atomically as one wire break**.
...
- **Frame constants:** `PROTOCOL_VERSION = 3`; `FRAME_TYPE_LEN = 1` (`FRAME_TYPE_HANDSHAKE = 1`, `FRAME_TYPE_CHUNK = 2`, `FRAME_TYPE_RECEIPT = 3`); `RATCHET_DHPUB_LEN = 32`, `RATCHET_N_LEN = 8`, `RATCHET_PN_LEN = 8`, `PQ_EPOCH_LEN = 1` (value 0 in v3); `CHUNK_HEADER_LEN = 49`; `MESSAGE_START = 50` (replaces the old 96 = ephemeral_pk32 + sig64).
- **Ratchet:** per `(roomId, peerPublicKey)` edge; advances per logical message; all chunks share one message key, nonce = chunkIndex; `MAX_SKIP = 512` bounds JS-side skipped-message-key derivations (anti-DoS); a retransmit resends the CACHED ciphertext (never re-encrypt with a fresh nonce, never reuse a message key across two messages).
...
- Constants in `src/utils/constants.ts` (Stage-1 SSOT): `CPACE_DOMAIN`, `KDF_RK_LABEL`, `KDF_CK_LABEL`, `KDF_MK_LABEL` (string values `"p2party-cpace-v1"`, `"p2party-rk-v1"`, `"p2party-ck-v1"`, `"p2party-mk-v1"`), `MAX_SKIP = 512`.
...
2. In `src/db/types.ts`, insert the `RatchetSession` interface immediately after the `SendQueue` interface (after line 105):
   ...
   export interface RatchetSession {
     ...
     }>; // capped at MAX_SKIP by the ratchet layer (Stage 2)
     updatedAt: number;
   }
   ```

...
- **DIVERGENCE from spec §6 — DECIDED: cut receive over to C.** T3-wiring left the receive crypto correct-but-split (AEAD decrypt = C `_decrypt_chachapoly_symmetric`; merkle walk = C `_verify_merkle_proof`; leaf hash = WebCrypto `crypto.subtle`; glued in TS across two C calls). USER WANTS C TO OWN THE MESSAGE CRYPTO. Fix (first task of the E2E session): rewire `handleReceiveMessage` to the already-built, already-tested `_receive_message_with_key(decrypted, frame, merkleRoot, messageKey)` — its AAD (`merkle_root‖N‖PN`) + `MESSAGE_START` offset are byte-identical to the v3 frame (`pake_ratchet.c:119-171`), so it does decrypt+leafhash+merkle in ONE C call. Keep ONLY the per-message-key derivation + clone/rollback in TS (ratchet state is a TS object). No WASM rebuild needed (`_receive_message_with_key` already exported). Verify with the existing `pake_ratchet.test.ts` + the E2E. This retires the WebCrypto leaf-hash on the receive path too.

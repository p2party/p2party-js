 ...
 * this compilation unit emits only ML-KEM-512-specific code.
 *
...
_Static_assert(MLKEM512_PUBLICKEYBYTES == P2PARTY_MLKEM512_PUBLICKEYBYTES,
               "ML-KEM-512 public-key size mismatch");
_Static_assert(MLKEM512_SECRETKEYBYTES == P2PARTY_MLKEM512_SECRETKEYBYTES,
               "ML-KEM-512 secret-key size mismatch");
_Static_assert(MLKEM512_CIPHERTEXTBYTES == P2PARTY_MLKEM512_CIPHERTEXTBYTES,
               "ML-KEM-512 ciphertext size mismatch");
_Static_assert(MLKEM_BYTES == P2PARTY_MLKEM512_BYTES,
               "ML-KEM shared-secret size mismatch");

#include "./vendor/mlkem-native/mlkem/mlkem_native.c"

int
mlkem512_keypair(uint8_t pk[P2PARTY_MLKEM512_PUBLICKEYBYTES],
                 uint8_t sk[P2PARTY_MLKEM512_SECRETKEYBYTES],
                 const uint8_t coins[P2PARTY_MLKEM512_KEYPAIRCOINBYTES])
{
  return p2party_mlkem512_keypair_derand(pk, sk, coins);
}

int
mlkem512_encaps(uint8_t ct[P2PARTY_MLKEM512_CIPHERTEXTBYTES],
...
{
  return p2party_mlkem512_enc_derand(ct, ss, pk, coins);
}

int
mlkem512_decaps(uint8_t ss[P2PARTY_MLKEM512_BYTES],
                const uint8_t ct[P2PARTY_MLKEM512_CIPHERTEXTBYTES],
                const uint8_t sk[P2PARTY_MLKEM512_SECRETKEYBYTES])
{
  return p2party_mlkem512_dec(ss, ct, sk);
}

...
const getDBChunk = async (merkleRootHex, chunkIndex) => {
  const db = await getDB();
  const chunk = await db.get("chunks", [merkleRootHex, chunkIndex]);
  db.close();
  return chunk?.data;
};
const existsDBChunk = async (merkleRootHex, chunkIndex) => {
  const db = await getDB();
  const count = await db.count("chunks", [merkleRootHex, chunkIndex]);
  db.close();
  return count > 0;
};
const getDBSendQueue = async (label, toPeerId) => {
  const db = await getDB();
...
const getDBAllChunks = async (merkleRootHex) => {
  const db = await getDB();
...
const getDBAllChunksCount = async (merkleRootHex) => {
  const db = await getDB();
  const chunksCount = await db.countFromIndex(
    "chunks",
    "merkleRoot",
...
const setDBChunk = async (chunk) => {
  const db = await getDB();
  await db.put("chunks", chunk);
  db.close();
};
const setDBSendQueue = async (chunk) => {
  const db = await getDB();
  await db.put("sendQueue", chunk);
  db.close();
};
const deleteDBChunk = async (merkleRootHex, chunkIndex) => {
  const db = await getDB();
  if (chunkIndex) {
...
const deleteDBSendQueueItem = async (position, label, toPeerId) => {
  const db = await getDB();

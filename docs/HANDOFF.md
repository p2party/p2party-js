...
- **Receive crypto now 100% libsodium/C (`58c08fb`).** T3-wiring left it correct but with the leaf hash in browser crypto; `hashMerkleLeafWasm` (`_sha512_*`) now does the receive-side leaf hash, so AEAD (`_decrypt_chachapoly_symmetric`) + merkle walk (`_verify_merkle_proof`) + leaf hash are all C. Only frame parse / ratchet bookkeeping / storage are TS. Send-side leaf hashing (`splitToChunks`, `getMerkleProof`) still WebCrypto — identical SHA-512, so proofs verify; migrate for symmetry if desired.
- **OPTIONAL consolidation (deferred, spec §6):** the single-call `_receive_message_with_key(decrypted, frame, merkleRoot, messageKey)` does decrypt+leafhash+merkle in ONE C call (AAD `merkle_root‖N‖PN` + `MESSAGE_START` already byte-match the v3 frame, `pake_ratchet.c:119-171`; return codes distinguish `-2` AEAD-fail → ratchet rollback from `-3/-6` merkle-fail → commit+drop). NOT taken yet because it collapses the cleanly-separated, 4-test `decryptMessageChunk` AEAD/ratchet primitive and needs a hand-built valid-merkle unit frame that only the E2E validates. Do it during the E2E cutover if you want the tighter single-call path; the crypto is already all-C without it.

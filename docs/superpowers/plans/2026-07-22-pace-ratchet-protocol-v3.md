...
- **Ratchet:** per `(roomId, peerPublicKey)` edge; advances per logical message; all chunks share one message key; each chunk carries a fresh **random 12-byte nonce in its cleartext frame header** (receiver-derivable, metadata-safe, birthday-safe within a per-message key); `MAX_SKIP = 512` bounds JS-side skipped-message-key derivations (anti-DoS); a retransmit resends the CACHED (nonce, ciphertext) pair (never re-encrypt with a fresh nonce, never reuse a message key across two messages).

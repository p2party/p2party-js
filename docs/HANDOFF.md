...
- **DX (user ask):** `generateKeyPair` now aliases `newKeyPair` (`d744f6b`, industry-standard). OPEN: the **message/ratchet crypto is not exposed** for standalone (non-WebRTC) use — only identity crypto is public; standalone use requires deep internal imports. Decide: expose raw primitives (`initRatchet`/`ratchetEncrypt`/`sealChunk`/`decryptMessageChunk`, quick) vs a designed `createSession().encrypt()/.decrypt()` wrapper (self-loads module + manages ratchet state/persistence; more work, safer API). `verify(message, signature, publicKey)` arg order differs from the common `(sig, msg, pk)` — noted, not changed.
